        ...
        {sideBarLink.map((link: ISideBarLink) => {
          return (
            <li className="w-full" key={link.label}>
              <NavLink
                className={({ isActive }) => {
                  return `flex pl-5 pt-5 pb-3  gap-4 text-sm leading-[34px] font-medium items-center w-full after:w-1 transition  after:h-9 after:content-[''] pr-1 after:rounded-full after:transition after:${
                    isActive ? "bg-white" : ""
                  } ${isActive ? "bg-[#01265F]" : ""}`;
                }}
                to={link.route}
